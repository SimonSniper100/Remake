...
import static carrier.Main.MathComplex.NonNegative;
...
public class ThursterEngine extends UnitType.UnitEngine{
    ...
    public void draw(Unit u){
        if(u.vel.len2()>=0.000001f){
            ...
            float z1 = Draw.z();
            if(z>0)Draw.z();
            float speed = u.moving() ? (u.vel.len()*1.05f/(u.speedMultiplier+u.dragMultiplier)):0;
            ...
            float 
                ...
                dx3= u.x +Angles.trnsx(u.rotation, y-lenght*(NonNegative(str))+1.5f*b,x+radius/2f),
                dx4= u.x +Angles.trnsx(u.rotation, y-lenght*(NonNegative(str))+1.5f*b,x-radius/2f);
            float 
                dy1= u.y +Angles.trnsy(u.rotation, y,x+radius/2f),
                dy2= u.y +Angles.trnsy(u.rotation, y,x-radius/2f),
                dy3= u.y +Angles.trnsy(u.rotation, y-lenght*(NonNegative(str))+1.5f*b,x+radius/2f),
                dy4= u.y +Angles.trnsy(u.rotation, y-lenght*(NonNegative(str))+1.5f*b,x-radius/2f);
